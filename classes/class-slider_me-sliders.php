...
/**
 * SliderMe Sliders Class
 *
 * All functionality pertaining to the slideshows in SliderMe.
 *
 * @package WordPress
 * @subpackage SliderMe
 * @category Core
...
class SliderMe_Sliders {
	...
	public function get_slides ( $type, $args = array(), $settings = array() ) {
		$slides = array();
		$supported_types = SliderMe_Utils::get_slider_types();
		...
		return (array) apply_filters( 'slider_me_get_slides', $slides, $type, $args, $settings );
	} // End get_slides()
			...
			foreach ( $attachments as $k => $v ) {
				...
				if ( 'true' == $args['thumbnails'] || 1 == $args['thumbnails'] ) {
					...
					if ( ! is_bool( $thumb_url ) ) {
						...
						$data['attributes'] = array( 'data-thumb' => esc_url( SliderMe_Utils::get_placeholder_image() ) );
					}
				}
				$slides[] = $data;
			}
	...
	private function slideshow_type_posts ( $args = array(), $settings = array() ) {
		...
		$supported_layouts = SliderMe_Utils::get_posts_layout_types();
		...
		if ( ! is_wp_error( $posts ) && ( count( $posts ) > 0 ) ) {
			...
			foreach ( $posts as $k => $post ) {
				...
				$layed_out_content = apply_filters( 'slider_me_posts_layout_html', $content, $args, $post );
	...
	private function slideshow_type_slides ( $args = array(), $settings = array() ) {
		...
		if ( ! is_wp_error( $posts ) && ( count( $posts ) > 0 ) ) {
			foreach ( $posts as $k => $post ) {
				...
				$data = array( 'content' => '<div class="slide-content">' . "\n" . apply_filters( 'slider_me_slide_content_slides', $content, $args ) . "\n" . '</div>' . "\n" );
				if ( 'true' == $args['thumbnails'] || 1 == $args['thumbnails'] ) {
					$thumb_url = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ), 'medium' );
					if ( ! is_bool( $thumb_url ) && isset( $thumb_url[0] ) ) {
						$data['attributes'] = array( 'data-thumb' => esc_url( $thumb_url[0] ) );
				...
				if ( 'true' == $settings['title_nav'] || 1 == $settings['title_nav'] ) {
					$data['attributes']['data-title'] = esc_attr( get_the_title() );
				}
				$slides[] = $data;
			}
			wp_reset_postdata();
	...
	public function apply_default_filters_slides () {
		add_filter( 'slider_me_slide_content_slides', 'wptexturize', 1 );
		add_filter( 'slider_me_slide_content_slides', 'convert_smilies', 1 );
		add_filter( 'slider_me_slide_content_slides', 'convert_chars', 1 );
		add_filter( 'slider_me_slide_content_slides', 'wpautop', 1 );
		add_filter( 'slider_me_slide_content_slides', 'shortcode_unautop', 1 );
		add_filter( 'slider_me_slide_content_slides', 'prepend_attachment', 1 );

		// Take note of the priority settings for the following filters.
		add_filter( 'slider_me_slide_content_slides', 'wp_kses_post', 2 );

		if ( get_option( 'embed_autourls' ) ) {
			global $wp_embed;
			add_filter( 'slider_me_slide_content_slides', array( &$wp_embed, 'run_shortcode' ), 3 );
		}

		add_filter( 'slider_me_slide_content_slides', 'do_shortcode', 4 );
