...
/**
 * SliderMe Widget Base Class
 *
 * Base widget class for SliderMe.
 *
 * @package WordPress
 * @subpackage SliderMe
 * @category Widgets
 ...
 * - generate_slideshow()
 */
class SliderMe_Widget_Base extends WP_Widget {

	/* Variable Declarations */
	protected $slider_type;
	protected $can_show_slideshow;
	protected $defaults = array( 'title' => '' );
	protected $slider_me_widget_cssclass;
	protected $slider_me_widget_description;
	protected $slider_me_widget_idbase;
	protected $slider_me_widget_title;
	...
	public function __construct () {
		...
		$this->slider_me_widget_cssclass = 'widget_slider_me_slideshow';
		$this->slider_me_widget_description = __( 'A slideshow of the content on your site', 'slider_me' );
		$this->slider_me_widget_idbase = 'slider_me_slideshow';
		$this->slider_me_widget_title = __( 'Slideshow (SliderMe)', 'slider_me' );
	...
	protected function init () {
		/* Widget settings. */
		$widget_ops = array( 'classname' => $this->slider_me_widget_cssclass, 'description' => $this->slider_me_widget_description );

		/* Widget control settings. */
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => $this->slider_me_widget_idbase );

		/* Create the widget. */
		$this->WP_Widget( $this->slider_me_widget_idbase, $this->slider_me_widget_title, $widget_ops, $control_ops );
	} // End init()
	...
	public function widget ( $args, $instance ) {
		...
		do_action( $this->slider_me_widget_cssclass . '_top' );
		...
		// Add actions for plugins/themes to hook onto.
		do_action( $this->slider_me_widget_cssclass . '_bottom' );
	...
	public function update ( $new_instance, $old_instance ) {
		...
		$fields = $slider_me->admin->generate_default_conditional_fields( array( $this->slider_type => $this->slider_type ) );
		...
		if ( isset( $instance['show_advanced_settings'] ) && ( $instance['show_advanced_settings'] == true ) ) {
			$slider_me->settings->init_fields();
			$advanced_fields = $slider_me->settings->fields;
		...
		$instance = apply_filters( $this->slider_me_widget_idbase . '_widget_save', $instance, $new_instance, $this );
   ...
   public function form ( $instance ) {
		...
		$defaults = apply_filters( $this->slider_me_widget_idbase . '_widget_defaults', $defaults, $this );
...
?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title (optional):', 'slider_me' ); ?></label>
			<input type="text" name="<?php echo $this->get_field_name( 'title' ); ?>"  value="<?php echo $instance['title']; ?>" class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" />
		</p>
		<?php if ( is_array( $themes ) && ( 1 < count( $themes ) ) ) { ?>
		<!-- Widget Theme: Select Input -->
		<p>
			<label for="<?php echo $this->get_field_id( 'theme' ); ?>"><?php _e( 'Slideshow Theme:', 'slider_me' ); ?></label>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'show_advanced_settings' ); ?>"><?php _e( 'Customise Advanced Settings', 'slider_me' ); ?></label>
		</p>
		<p><small><?php _e( 'Save the widget settings with this checkbox checked to customise slideshow settings.', 'slider_me' ); ?></small></p>
		<?php
...
<?php
		// Allow child themes/plugins to act here.
		do_action( $this->slider_me_widget_idbase . '_widget_settings', $instance, $this );
